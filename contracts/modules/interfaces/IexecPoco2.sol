...
interface IexecPoco2
{
	...
	event TaskExtended  (bytes32 indexed taskid, uint256 finalDuration);
	event TaskInterrupt (bytes32 indexed taskid, uint256 finalDuration);
	...
	function extend(bytes32,uint256) external;
	function interrupt(bytes32) external;
}
