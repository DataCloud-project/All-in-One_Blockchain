...
contract IexecPoco2Delegate is IexecPoco2, DelegateBase, IexecERC20Core, SignatureVerifier
{
	...
	public override
	{
		...
		require(deal.category == 5, 'Task extension is only for service tasks');
		 ...
		 * Stake management
		 */
		uint256 extendedPrice = deal.workerpool.price.mul(extendedDuration);
		lock(
			deal.requester,
			extendedPrice
		);

		emit TaskExtended(_taskid, deal.duration);
	}
	...
	public override
	{
		...
		emit TaskInterrupt(_taskid, deal.duration);
	}
}
