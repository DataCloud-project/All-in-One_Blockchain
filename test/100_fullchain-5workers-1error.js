...
contract('Fullchain', async (accounts) => {
	...
	before("configure", async () => {
		...
		workers = [
			...
			{ agent: worker2, useenclave: true,  result: "je ne veux pas travailler" },
			{ agent: worker3, useenclave: true,  result: "iExec the wanderer" },
			{ agent: worker4, useenclave: true,  result: "iExec the wanderer" },
			{ agent: worker5, useenclave: true,  result: "iExec the wanderer" },
		];
	...
	describe("→ pipeline", async () => {
		...
		describe("[5] finalization", async () => {
			it("[TX] finalize", async () => {
				...
				// TODO: check 2 events by w.agent.address for w in workers
	...
	describe("→ summary", async () => {
		...
		it("balances", async () => {
			...
			assert.deepEqual(await scheduler.viewAccount(),       [ 1005, 0 ], "check balance");
			assert.deepEqual(await worker1.viewAccount(),         [ 1007, 0 ], "check balance");
			assert.deepEqual(await worker2.viewAccount(),         [  992, 0 ], "check balance");
			assert.deepEqual(await worker3.viewAccount(),         [ 1007, 0 ], "check balance");
			assert.deepEqual(await worker4.viewAccount(),         [ 1007, 0 ], "check balance");
			assert.deepEqual(await worker5.viewAccount(),         [ 1007, 0 ], "check balance");
