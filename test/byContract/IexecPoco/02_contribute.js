...
contract('Poco', async (accounts) => {
	...
	it("[2.4] Contribute - Error (duplicate)", async () => {
		...
		results = odbtools.utils.sealResult(__taskid, __raw, __worker.address),
		authorization = await odbtools.utils.signAuthorization({ worker: __worker.address, taskid: __taskid, enclave: __enclave.address }, scheduler.wallet);
		// First ok
	...
	it("[2.5] Contribute - Error (authorization)", async () => {
		...
		await expectRevert.unspecified(sendContribution(
			...
			__worker,
			odbtools.utils.sealResult(__taskid, __raw, __worker.address),
			await odbtools.utils.signAuthorization({ worker: __worker.address, taskid: __taskid, enclave: __enclave.address }, __worker.wallet), // signature: scheduler → worker
			__enclave.address
		));
	});
