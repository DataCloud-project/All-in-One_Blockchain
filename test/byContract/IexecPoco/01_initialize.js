...
contract('Poco', async (accounts) => {
	...
	it("[Setup]", async () => {
		...
		serviceworkerpoolorder = await scheduler.signWorkerpoolOrder({
			workerpool:        WorkerpoolInstance.address,
			workerpoolprice:   25,
			hardware:		   '<hardware>',
			volume:            1000,
		...
		servicerequestorder = await user.signRequestOrder({
			app:                AppInstance.address,
			appmaxprice:        3,
			dataset:            DatasetInstance.address,
			datasetmaxprice:    1,
		...
		await Promise.all([
			...
			IexecInstance.matchOrders(apporder, datasetorder, serviceworkerpoolorder,        servicerequestorder, { from: user.address }),
		]);

		deals = await odbtools.utils.requestToDeal(IexecInstance, odbtools.utils.hashRequestOrder(ERC712_domain, requestorder));
		servicedeals = await odbtools.utils.requestToDeal(IexecInstance, odbtools.utils.hashRequestOrder(ERC712_domain, servicerequestorder));
	});
	...
	it("[1.5] Initialization - Correct (service)", async () => {
		txMined = await IexecInstance.initialize(servicedeals[0], 1, { from: scheduler.address });
		events = tools.extractEvents(txMined, IexecInstance.address, "TaskInitialize");
		assert.equal(events[0].args.workerpool, WorkerpoolInstance.address, "check workerpool");
	});
