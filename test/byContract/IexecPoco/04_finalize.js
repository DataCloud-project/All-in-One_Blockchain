...
contract('Poco', async (accounts) => {
	...
	it("[setup] Initialization", async () => {
		tasks[1] = tools.extractEvents(await IexecInstance.initialize(deals[1], 1, { from: scheduler.address }), IexecInstance.address, "TaskInitialize")[0].args.taskid; // full
		tasks[2] = tools.extractEvents(await IexecInstance.initialize(deals[1], 2, { from: scheduler.address }), IexecInstance.address, "TaskInitialize")[0].args.taskid; // partial to soon
		tasks[3] = tools.extractEvents(await IexecInstance.initialize(deals[1], 3, { from: scheduler.address }), IexecInstance.address, "TaskInitialize")[0].args.taskid; // partial ok
		tasks[4] = web3.utils.soliditySha3({ t: 'bytes32', v: deals[1] }, { t: 'uint256', v: 4 });                                                                // uninitialized
		tasks[5] = tools.extractEvents(await IexecInstance.initialize(deals[1], 5, { from: scheduler.address }), IexecInstance.address, "TaskInitialize")[0].args.taskid; // no consensus
		tasks[6] = tools.extractEvents(await IexecInstance.initialize(deals[1], 6, { from: scheduler.address }), IexecInstance.address, "TaskInitialize")[0].args.taskid; // no reveal
	...
	it("[setup] Contribute", async () => {
		...
		await sendContribution(
			...
			await odbtools.utils.signAuthorization({ worker: worker2.address, taskid: tasks[3], enclave: constants.NULL.ADDRESS }, scheduler.wallet),
			constants.NULL.ADDRESS
		);
	...
	it("[setup] Reveal", async () => {
		...
		await IexecInstance.reveal(tasks[1], odbtools.utils.hashResult(tasks[1], "true").digest, { from: worker2.address });
		...
		await IexecInstance.reveal(tasks[7], odbtools.utils.hashResult(tasks[7], "true").digest, { from: worker1.address });
	});
