...
contract('Fullchain', async (accounts) => {
	...
	before("configure", async () => {
		...
		workers = [
			...
			{ agent: worker2, useenclave: true,  result: "iExec the wanderer" },
	...
	describe("→ summary", async () => {
		...
		it("balances", async () => {
			...
			assert.deepEqual(await worker1.viewAccount(),         [ 1005, 0 ], "check balance");
			assert.deepEqual(await worker2.viewAccount(),         [ 1005, 0 ], "check balance");
			assert.deepEqual(await worker3.viewAccount(),         [ 1005, 0 ], "check balance");
			assert.deepEqual(await worker4.viewAccount(),         [ 1005, 0 ], "check balance");
			assert.deepEqual(await worker5.viewAccount(),         [ 1000, 0 ], "check balance");
