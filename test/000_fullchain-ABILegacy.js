		...
		describe("[1] order matching", async () => {
			...
			describe("checks", async () => {
				...
				it("config", async () => {
					...
					assert.equal  (config[3].toNumber(), 0,                        "check config (config.botFirst)"            );
					assert.equal  (config[4].toNumber(), 1,                        "check config (config.botSize)"             );
					assert.equal  (config[5].toNumber(), 8,                        "check config (config.workerStake)"         ); // 8 = floor(25*.3)
					assert.equal  (config[6].toNumber(), 5,                        "check config (config.schedulerRewardRatio)");
				});
